...
async def import_acc(
    file_name: str = "import", separator: str = ":", form: str = "email:password"
):
    ...
    for line in lines:
        ...
        if await validate_params(set(keys), line):
            account = tuple(
                [line[keys.index(i)] if i in keys else None for i in ALLOW_PARAM]
            )
    ...
    if len(list_accounts) > 0:
        ...
        logger.error(
            f"Error in the parameters, separator or fromat not equal import data\nALWAYS PARAMETERS: {ALWAYS_PARAM}\nALLOW_PARAMETERS: {ALLOW_PARAM}"
        )
...
async def export_acc(
    ...
    if export_type == "txt":
        ...
        for acc in list_accs:
            data = [
                acc.get(key, "") if acc.get(key, "") != None else "None" for key in keys
            ]
            if len(data) > 1:
