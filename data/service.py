...
def generate_pass():
    ...
    password += random.choices(
        string.ascii_letters + string.digits + symbols, k=remaining_length
    )
...
def get_user_agent():
    if USER_AGENTS:
        ...
        logger.debug(f"Generate user-agent list")
        for _ in range(150):
            USER_AGENTS.append(
                UserAgent("Chrome", "Windows", 131.0, 0.0, "desktop").random
            )
        return random.choice(USER_AGENTS)
...
def get_random_mail(n):
    ...
    for _ in range(n):
        ...
        local_part = "".join(
            random.choices(string.ascii_letters + string.digits, k=length_local)
        )
        ...
        if domain_list:
            ...
            domain_part = (
                "".join(random.choices(string.ascii_lowercase, k=length_domain))
                + ".com"
            )
...
def get_ref(path_ref: str = "referral.txt"):
    ...
    with open(path_ref, "r", encoding="utf-8") as file:
        ...
        if len(ref) == 0:
            ref = ["", ""]
    logger.debug(f"Get ref code: {ref}")
    return ref


def get_nickname():
    nickname = (
        RandomNicknames().random_nick(gender="m").lower()
        + "".join(
            random.choice(string.ascii_lowercase) for _ in range(random.randint(1, 3))
        )
        + random.choice(
            [
                "",
        ...
        with open(path, "r", encoding="utf-8") as file:
            for string in file.readlines():
                ...
                if ":" in string:
                    ...
                    if validate_email(string[0]) and (
                        ...
                        imap_domain = (
                            string[2]
                            if len(string) == 3
                            else fixed_imap if fixed_imap else None
                        )
