                    ...
                    for i in folders:
                        ...
                        if status == 'OK' and search != [None]:
                            ...
                            if 'Your One Time Password for Grass is' in request:
                                status, data = mailbox.fetch(last_mail, '(BODY[HEADER.FIELDS (SUBJECT)])')
...
def check_trash_grass(mailbox: SocksIMAP4SSL):
    ...
    try:
        typ, data = mailbox.list()
        folders = [re.search(br'["|/] (.+)', folder).group(1).decode('utf-8') for folder in data]
        ...
        for folder in folders:
            ...
            if status == 'OK' and search[0]:
                ...
                if len(msg_ids) > 20:
                    logger.info(f'Found {len(msg_ids)} "Grass Foundation" emails in {folder}, deleting...')

                    for msg_id in msg_ids:
                        mailbox.store(msg_id, '+FLAGS', '\\Deleted')
                    mailbox.expunge()
                    logger.info(f'Successfully deleted Grass Foundation emails from {folder}')
    except Exception as e:
